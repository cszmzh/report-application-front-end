    ...
    $.ajax({
        ...
        success:function(res){
            ...
            for(var i=0; i<list.length; i++){
                ...
                str+="<tr><td>"+(pageSize*(page-1)+i+1)+"</td><td style=\"cursor:pointer\" class='detail_info' >"+list[i].stdId+"</td><td>"+list[i].stdName+"</td><td>"+list[i].major+"</td><td>" +
            ...
            for(var i=1; i<=res.data.total/pageSize+1;i++){
...
$('body').on("click",".detail_info",function (e) {
    window.location.href='reportDetail.html?stdId='+ $(e.target).context.innerHTML;
});
...
$('body').on("click","#findByIdButton",function (e) {
    ...
    $.ajax({
        url:backgroundURL+'report/getByStdId?stdId='+ $('#findByIdContent').val(),
        type:'post',
        datatype:'json',
        success:function(res){
            ...
            //遍历数据
                ...
                str+="<tr><td>1</td><td style=\"cursor:pointer\" class='detail_info' >"+data.stdId+"</td><td>"+data.stdName+"</td><td>"+data.major+"</td><td>" +
